...
ctx logger info "Format HDFS"
## Format HDFS NN
sudo -u hdfs hdfs namenode -format -force

ctx logger info "Start HDFS"
## Start HDFS services
for service in $(cd /etc/init.d; ls hadoop-hdfs-*); do
    sudo service ${service} start
done

ctx logger info "Waiting for HDFS"
while [[ $(nc -z $(hostname) 8020; echo $?) -ne 0 ]]; do
    sleep 5
done

ctx logger info "Directories"
## Directories & permissions
sudo -u hdfs hadoop fs -mkdir -p /tmp
sudo -u hdfs hadoop fs -chmod -R 1777 /tmp
sudo -u hdfs hadoop fs -mkdir -p /user/history
sudo -u hdfs hadoop fs -chmod -R 1777 /user/history
sudo -u hdfs hadoop fs -chown mapred:supergroup /user/history

ctx logger info "Create user dir"
## Create MapReduce user directories
# For now just us, but which are all the users that need perms?
# e.g. history created before but what else?
if [[ ${is_namenode} == 'True' ]]; then
    sudo -u hdfs hadoop fs -mkdir -p /user/$(whoami)
    sudo -u hdfs hadoop fs -chown $(whoami):supergroup /user/$(whoami)
fi
...
ctx logger info "Waiting for Spark"
while [[ $(nc -z $(hostname) 7077; echo $?) -ne 0 ]]; do
    sleep 5
done
