...
with FileLock(lock_file, lock_details=lock_details) as lock:
    ...
    if tgt['ip'] not in src['worker_ips']:
        src['worker_ips'] += [tgt['ip']]
    else:
        raise NonRecoverableError('worker_on_config: duplicate ip entry')

    hostname = tgt['ip'].replace('.', '-')
    if hostname not in src['worker_fqdns']:
        src['worker_fqdns'] += [hostname]
    else:
        raise NonRecoverableError('worker_on_config: duplicate FQDN entry')

    ctx.source.instance.update()

    lock.log('config_on_worker: {}'.format(tgt['ip']))
    lock.log('config_on_worker: {}'.format(src['worker_ips']))
    lock.log('config_on_worker: {}'.format(hostname))
    lock.log('config_on_worker: {}'.format(src['worker_fqdns']))
